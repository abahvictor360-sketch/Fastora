...
const nextConfig = {
   ...
   *
   * Next defaults to one worker per CPU, which on the deploy host meant 19 of
   * them requesting from the Laravel backend at once. That backend is on shared
   * hosting, and it answered a share of that burst with 500s — the same paths
   * served fine seconds later, so this is load, not broken content.
   *
   * The site is thirty-odd pages. Generating them a few at a time costs a few
   * seconds of build time and keeps the request rate to something shared
   * hosting can serve; staticGenerationRetryCount then gives any page that
   * still trips a second attempt before the build gives up.
   */
  experimental: {
